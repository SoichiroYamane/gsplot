...
def main() -> None:
    ...
    for i, (gap_values, heat_values, name) in enumerate(zip(gap, heat_data, LABELS)):
        ...
        options = {"series": i, "label": name, "ms": 0, "lw": 2}
        gs.line(ax["A"], *gap_values, **options)
        gs.line((ax["B"], heat), temperature, capacity, **options)
        gs.line(square, temperature**2, capacity, **options)
    for values, name in zip(yosida, EVEN):
        i = NAMES.index(name)
        gs.line(ax["C"], *values, series=i, label=LABELS[i], ms=0, lw=2)
    gs.legend(ax["A"], loc="lower left", handlelength=3)
