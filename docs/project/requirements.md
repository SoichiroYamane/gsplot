...
- The `main` branch must require a pull request, the named CI and security
  ...
  approval of the latest push. Force-pushes and branch deletion must be
  blocked. When at least two maintainers are available, require at least one
  independent GitHub approval. With one maintainer, the required approval
  count may be zero; self-approval and administrative bypass are prohibited,
  and the public maintenance protocol still requires Review 1 and Review 2
  before handoff.
