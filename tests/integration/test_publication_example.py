...
EXAMPLE_PATH = (
    Path(__file__).resolve().parents[2] / "examples" / "publication" / "publication.py"
)


def _load_example() -> ModuleType:
    """Load the example without executing its guarded entry point."""

    spec = spec_from_file_location("gsplot_publication_example", EXAMPLE_PATH)
    if spec is None or spec.loader is None:  # pragma: no cover - importlib guard
        raise RuntimeError("could not load the publication example")
    module = module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def test_publication_example_uses_balanced_canvas_and_closes_figure(
    ...
    monkeypatch.setattr(example, "ROOT", tmp_path)
    monkeypatch.setattr(example.gs, "save", fake_save)
    example.main()
    ...
    gap = [
        np.genfromtxt(
            example.DATA / "gap" / f"Gapeq_{name}.dat",
            delimiter="\t",
            skip_header=1,
            unpack=True,
        )
    ...
    capacity = [
        np.genfromtxt(
            example.DATA / "heat_capacity" / f"C_{name}.dat",
            delimiter="\t",
            skip_header=1,
            unpack=True,
        )
    ...
    yosida = [
        np.genfromtxt(
            example.DATA / "yosida" / f"Y(T)_{name}.dat",
            delimiter="\t",
            skip_header=1,
            unpack=True,
        )
        for name in example.EVEN
    ]
    ...
    assert [text.get_text() for text in axes["C"].get_legend().texts] == [
        example.LABELS[example.NAMES.index(name)] for name in example.EVEN
    ]
...
def test_publication_example_closes_figure_when_output_fails(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """A failed export does not leak the Figure owned by the example."""
    ...
    monkeypatch.setattr(example, "ROOT", tmp_path)
    monkeypatch.setattr(example.gs, "save", fail_save)
